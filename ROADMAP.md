...
4. **Task manager (`src/tasks.rs`).** ✅ **seam built.** `tasks::spawn(work,
   ...
   *not* `Send` and run on the main thread, so they may touch widgets and
   ...
   window close. Converted: dictionary import (was freezing the UI) and the
   thumbnail backfill (now cancellable). Still to migrate: metadata fetch and
   the remaining `thread::spawn` sites, then downloads when P6 lands.
