...
engine) belongs to it — resolved direction: **no browse mode**; WebKit =
EPUB engine only (lazy), custom renderer for source-fetched fiction +
comics; EPUB normalization engine (crengine-style) is the later "Path B"
crown (see `docs/conversation.md` §7–§8).
