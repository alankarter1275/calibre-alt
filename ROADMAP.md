...
- ⬜ Next: the relaunch flow (selecting currently takes effect on next start),
  migrating the existing library, the per-library/global split, and
  `kalam.json` per book.

**A note on how this went:** the first two pushes failed CI on dead code —
`-D warnings` rejected the registry's methods for having no caller, which is
exactly the rule `source-seam.md` §12 describes for the `Source` trait. The fix
was to build the settings card rather than silence the warning, which is the
right pressure: it means no API lands here without something that uses it.
